...
| [cve-lookup](https://github.com/nlink-jp/cve-lookup) | Context for a CVE from the EchelonGraph CVE Pulse public API — CVSS by version, CISA KEV, EPSS, SSVC, exploit and patch signals, CPE product matching, vendor advisories (many predate the CVE ID) and the internet-exposure footprint; "upstream has no data" is never passed on as a negative; no key or account, the licence-free sibling of gti-lookup and a dependency-free replacement for the vendor's npx-launched MCP server; CLI + local MCP server (Go) |
| [gti-lookup](https://github.com/nlink-jp/gti-lookup) | Threat context from Google Threat Intelligence — association context for an indicator, sandbox behaviour served index-first, GTI-syntax IOC corpus search, the vulnerability catalogue with ATT&CK trees, and read-only LiveHunt ruleset inspection; ships the GTI Standard feature set; CLI + local MCP server, commercial GTI licence key required (Go) |
